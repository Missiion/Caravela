...
(function () {
    ...
    function openGame() {
        ...
        playSfx('start');
        // Desabilitar o banner enquanto o jogo está aberto
    ...
    function closeGame() {
        stopPhysics();
        // Re-habilitar o banner quando o jogo fecha
        const wrap = document.querySelector('.suika-banner-wrap');
    ...
    async function renderLeaderboard(filter) {
        ...
        list.innerHTML = entries.map((e) => {
            ...
            return `<div class="suika-lb-row ${cls}"><span class="suika-lb-rank">${medal}</span><span class="suika-lb-name">${e.name}</span><span class="suika-lb-score">${e.score.toLocaleString()}</span></div>`;
        }).join("");
    }
